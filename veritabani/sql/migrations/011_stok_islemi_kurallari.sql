-- =========================================================
-- Migration 011: stok işlemi kuralları — iade karşı tarafı ve amaç/lokasyon tipi
--
-- İki eksik iş kuralını `stok_islemi_kaydet()` içine ekler. İkisi de bugün ne
-- arayüzde ne veritabanında var; sıradaki arayüz turu bunları "ayna" olarak
-- göstereceği için önce burada tanımlanıyorlar.
--
-- 1) İadelerde karşı taraf zorunlu VE rollü:
--    MUSTERI_IADE   -> aktif MUSTERI rolü olan iş ortağı,
--    TEDARIKCI_IADE -> aktif TEDARIKCI rolü olan iş ortağı.
--    Defter etkileri (giriş/çıkış) 008'den beri doğrulanıyordu; eksik olan
--    karşı tarafın kendisiydi. Müşterisi yazılmamış bir iade kaydı işe yaramaz.
--    Mevcut SATIN_ALMA_KABUL / SATIS_SEVKI kontrollerinin biçimi ve mesaj
--    üslubu birebir izlendi; ikinci bir doğrulama deseni açılmadı.
--
-- 2) Amaç ↔ lokasyon tipi:
--    SATIN_ALMA_KABUL, URETIM_GIRIS, MUSTERI_IADE -> hedef DAHILI
--    SATIS_SEVKI, TEDARIKCI_IADE                  -> kaynak DAHILI
--    SAYIM                                        -> hedef DAHILI veya NUMUNE
--    IC_TRANSFER                                  -> iki uç da DAHILI/NUMUNE
--    DUZELTME                                     -> KISIT YOK (bilerek)
--    FASON_SEVK / FASON_DONUS / FIRE              -> mevcut kontroller aynen
--
--    DUZELTME'nin dışarıda bırakılması bir eksiklik değil karardır: fason
--    lokasyonundaki bir hatanın da düzeltilebilmesi gerekiyor, oraya kısıt
--    koymak düzeltme akışını kapatırdı. STOK_SINIFLANDIRMA ve MIRAS_HAREKET
--    de kapsam dışıdır.
--
-- ---------------------------------------------------------
-- Yöntem
-- ---------------------------------------------------------
-- İmza DEĞİŞMİYOR, `CREATE OR REPLACE` yeterli — 010'daki RENAME desenine
-- burada gerek yok. Gövde 008'den birebir alınıp yalnız bu iki kural eklendi;
-- başka hiçbir satır değiştirilmedi. Rollback dosyası 008 gövdesini birebir
-- geri yazar.
--
-- Kontroller mevcut satır döngüsünde, lokasyonun yaprak/aktif denetiminin
-- yanında duruyor; ikinci bir geçiş açılmadı. SAYIM için yer kritiktir:
-- sayım farkı ilerideki bir adımda kaynak/hedefi yeniden yazdığı için denetim
-- o yeniden yazmadan ÖNCE çalışmalıdır.
--
-- ---------------------------------------------------------
-- Geçmiş veri
-- ---------------------------------------------------------
-- Backfill YOK; kurallar yalnız yeni yazımlarda çalışır. Uygulama öncesi
-- ölçüm (2026-08-06, salt-okunur, hem yerel Docker hem Raspberry Pi):
-- her iki kurala da aykırı hareket/belge sayısı SIFIR. Defterdeki hareketlerin
-- neredeyse tamamı MIRAS_HAREKET nedenli (bu neden kapsam dışı), kalan tek
-- URETIM_GIRIS hareketi zaten DAHILI hedefe yazılmış.
--
-- ---------------------------------------------------------
-- Sıra
-- ---------------------------------------------------------
-- 011, 010'un ÜSTÜNE gelir. 010 da henüz ortak veritabanına uygulanmadı;
-- ikisi Django hizalamasıyla birlikte planlanmalıdır.
--
...
BEGIN
    ...
    END IF;
    -- 011: iadeler de karşı taraf ister. Müşterisi yazılmamış bir iade kaydı
    -- işe yaramaz; alış/satışla aynı zorunluluk iade yönünde de geçerli.
    IF p_islem_nedeni IN ('SATIN_ALMA_KABUL', 'SATIS_SEVKI',
                          'MUSTERI_IADE', 'TEDARIKCI_IADE')
    ...
    END IF;
    -- 011: iade yönü rolü de belirler — müşteri iadesini tedarikçi rolüyle,
    -- tedarikçi iadesini müşteri rolüyle kaydetmek mümkün olmamalı.
    IF p_islem_nedeni = 'MUSTERI_IADE' AND NOT EXISTS (
        SELECT 1 FROM is_ortaklari io
        JOIN is_ortagi_rolleri r USING (is_ortagi_id)
        WHERE io.is_ortagi_id = p_is_ortagi_id AND io.aktif_mi AND r.rol = 'MUSTERI'
    ) THEN
        RAISE EXCEPTION 'Müşteri iadesinde aktif bir müşteri seçilmelidir.';
    END IF;
    IF p_islem_nedeni = 'TEDARIKCI_IADE' AND NOT EXISTS (
        SELECT 1 FROM is_ortaklari io
        JOIN is_ortagi_rolleri r USING (is_ortagi_id)
        WHERE io.is_ortagi_id = p_is_ortagi_id AND io.aktif_mi AND r.rol = 'TEDARIKCI'
    ) THEN
        RAISE EXCEPTION 'Tedarikçi iadesinde aktif bir tedarikçi seçilmelidir.';
    END IF;
    ...
    LOOP
        ...
        -- 011: amaç ↔ lokasyon tipi. Yaprak/aktif denetiminin hemen yanında,
        -- aynı satır döngüsünde; ikinci bir geçiş açılmıyor.
        -- DUZELTME bilerek dışarıda: fason lokasyonundaki bir hatanın da
        -- düzeltilebilmesi gerekiyor, buraya kısıt koymak o akışı kapatırdı.
        -- FASON_SEVK / FASON_DONUS / FIRE'ın kendi kontrolleri aşağıda durur.
        -- SAYIM'da bu denetim, sayım farkının kaynak/hedefi yeniden yazmasından
        -- ÖNCE çalışmalıdır; yeri bu yüzden burasıdır.
        IF p_islem_nedeni IN ('SATIN_ALMA_KABUL', 'URETIM_GIRIS', 'MUSTERI_IADE')
           AND v_hedef IS NOT NULL AND NOT EXISTS (
            SELECT 1 FROM lokasyonlar WHERE lokasyon_id = v_hedef AND tip = 'DAHILI'
        ) THEN
            RAISE EXCEPTION 'Satır %: bu işlem nedeni yalnız dahili lokasyona giriş yapabilir.', v_sira;
        END IF;
        IF p_islem_nedeni IN ('SATIS_SEVKI', 'TEDARIKCI_IADE')
           AND v_kaynak IS NOT NULL AND NOT EXISTS (
            SELECT 1 FROM lokasyonlar WHERE lokasyon_id = v_kaynak AND tip = 'DAHILI'
        ) THEN
            RAISE EXCEPTION 'Satır %: bu işlem nedeni yalnız dahili lokasyondan çıkış yapabilir.', v_sira;
        END IF;
        IF p_islem_nedeni = 'SAYIM'
           AND v_hedef IS NOT NULL AND NOT EXISTS (
            SELECT 1 FROM lokasyonlar
            WHERE lokasyon_id = v_hedef AND tip IN ('DAHILI', 'NUMUNE')
        ) THEN
            RAISE EXCEPTION 'Satır %: sayım yalnız dahili veya numune lokasyonunda yapılabilir.', v_sira;
        END IF;
        IF p_islem_nedeni = 'IC_TRANSFER' AND (
            (v_kaynak IS NOT NULL AND NOT EXISTS (
                SELECT 1 FROM lokasyonlar
                WHERE lokasyon_id = v_kaynak AND tip IN ('DAHILI', 'NUMUNE')))
            OR (v_hedef IS NOT NULL AND NOT EXISTS (
                SELECT 1 FROM lokasyonlar
                WHERE lokasyon_id = v_hedef AND tip IN ('DAHILI', 'NUMUNE')))
        ) THEN
            RAISE EXCEPTION 'Satır %: iç transferin iki ucu da dahili veya numune lokasyonu olmalıdır.', v_sira;
        END IF;
