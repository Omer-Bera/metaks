...
- **`stok_hareketleri`**: ledger table (islem_tipi includes `SAYIM_DEVRI` for physical inventory counts, `gecici_kod` field explicitly for temporary/unreconciled codes) — schema anticipates the Faz 4 warehouse-count workflow but no loader script exists yet. `lokasyonlar` has 3 seed/test rows as of 2026-07-29 (`Ana Depo`/`Sevkiyat Alanı` DAHILI, `Fason Atölye 1` FASON) so Appsmith's `LokasyonSelect` has something to test against — not real business locations, replace/expand when known. `stok_hareketleri` itself is still empty (0 rows).
- **`stok_hareketleri.yapan_kullanici`**: not applied yet — designed in `sql/migrations/003_stok_hareketi_fonksiyonu.sql` (NOT NULL, no default; every insert must go through `stok_hareketi_kaydet()`, which takes it as a required parameter sourced from Appsmith's `{{ appsmith.user.email }}`, since all Appsmith traffic shares one Postgres role and `current_user` can't distinguish end users).
...
- **Stock-quantity views/function — prepared, NOT yet applied**: `sql/migrations/002_lokasyon_stok_view.sql` (`v_lokasyon_stok_ozet`, `v_toplam_stok` — will return 0 rows until Faz 4 count data exists, that's expected) and `003_stok_hareketi_fonksiyonu.sql` (`stok_hareketi_kaydet()` — the only sanctioned way to write to `stok_hareketleri`; enforces sufficient-stock checks, required `yapan_kullanici`, per-`islem_tipi` location requirements, and idempotency via a client-generated `istemci_islem_kimligi` UUID). Both syntax/logic-tested against the live schema inside a `BEGIN...ROLLBACK` (never committed) before being written — do the same before proposing further changes to them. Ask the user before actually applying either.
