...
- **`stok_hareketleri`**: ledger table (islem_tipi includes `SAYIM_DEVRI` for physical inventory counts, `gecici_kod` field explicitly for temporary/unreconciled codes) — schema anticipates the Faz 4 warehouse-count workflow but no loader script exists yet. `lokasyonlar` — ✅ **real business locations entered 2026-07-30**: `Metaks`/`Depo 1`/`Fabrika` (DAHILI), `Kaplama`/`Skor` (FASON). The original 3 seed/test rows (`Ana Depo`/`Sevkiyat Alanı`/`Fason Atölye 1`) were soft-deactivated (`aktif_mi = false`), not hard-deleted — `stok_hareketleri`'s 21 test rows from 2026-07-29 still FK-reference them (`ON DELETE RESTRICT`), and Appsmith's location dropdowns already filter `WHERE aktif_mi = true`, so they're just gone from the UI. `stok_hareketleri` itself has those 21 test rows still in it — user intends to clean them up later ("en son temizleriz"), not done yet.
