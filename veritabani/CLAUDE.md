...
- **Git**: initialized 2026-07-28, local repo-scoped `user.name`/`user.email` (not global). Remote added 2026-07-29: `https://github.com/Omer-Bera/metaks_DB` (private). Branch structure mirrors `depo-appsmith-arayuz` (see "UI / arayüz katmanı" below): `master` (stable), `Ömer`/`Furkan` (each person's own work), `Ömer-Claude`/`Furkan-Claude` (each person's Claude Code session). Currently on `Ömer-Claude`. No branch protection yet, same rationale as the other repo (early/fast phase). Furkan still needs to be added as a GitHub collaborator on this repo separately from `depo-appsmith-arayuz` if/when he starts touching data-infra work.
